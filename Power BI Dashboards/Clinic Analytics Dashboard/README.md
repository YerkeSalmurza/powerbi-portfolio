...
Data Last Update = 
FORMAT(
    MAX('V_FINANCE_ATLAS_UNIQUE'[DAT_SERV]),
    "dd.mm.yy", "ru-RU")
```
```dax
PBI Refresh Date = 
...
```dax
Tooltip ГКП = 
"Кол-во услуг — сумма всех оказанных услуг сотрудникам ГКП за выбранный период." 
& UNICHAR(10) & UNICHAR(10) &
"Стоимость услуг — суммарная стоимость оказанных услуг сотрудникам ГКП за выбранный период (в тыс. руб.)."
& UNICHAR(10) & UNICHAR(10) &
"Доля сотрудников ГКП — процент сотрудников ГКП, получивших хотя бы одну услугу в выбранный период, от общего числа всех сотрудников ГКП."
```
...
```dax
Tooltip Депозиты = 
"Кол-во депозитов — количество размещённых депозитов за выбранный период (учитываются только основные взносы)."
& UNICHAR(10) & UNICHAR(10) &
"Стоимость депозитов — суммарная сумма денежных средств, внесённых на депозиты за выбранный период (в тыс. руб.)."
& UNICHAR(10) & UNICHAR(10) &
"Эффективность депозитов — отношение стоимости услуг, оплаченных за счёт депозитов, к общей сумме внесённых депозитов."
```
...
```dax
Tooltip Продажи КП = 
"Кол-во КП — количество уникальных проданных комплексных программ за выбранный период."
& UNICHAR(10) & UNICHAR(10) &
"Стоимость КП — суммарная выручка от продажи комплексных программ за выбранный период (в тыс. руб.)."
& UNICHAR(10) & UNICHAR(10) &
"Доля КП в выручке АМБ — отношение выручки от КП к общей выручке амбулаторного направления."
```
```dax
Tooltip ТОП-10 КП = 
"Рейтинг комплексных программ по выручке за выбранный период."
& UNICHAR(10) & UNICHAR(10) &
"Стоимость КП — суммарная выручка от продажи программы (в тыс. руб.)."
& UNICHAR(10) & UNICHAR(10) &
"Кол-во КП — количество проданных программ за выбранный период."
```
```dax
Tooltip Длит Ведение = 
"Количество уникальных клиентов, получающих услуги в рамках программ длительного ведения «Врач в друзьях» (КП-224–230) в отчётном месяце."
```
```dax
Tooltip Доля Активных КП = 
"Доля активных КП — отношение количества комплексных программ, по которым были продажи в периоде, к общему количеству программ в прейскуранте."
& UNICHAR(10) & UNICHAR(10) &
"Показатель отображается поквартально."
```
```dax
Tooltip Эффективность Чекапов = 
"Конверсия в депозиты — доля клиентов, купивших КП и оформивших депозит в том же периоде."
& UNICHAR(10) & UNICHAR(10) &
"Конверсия в допродажи — доля клиентов, купивших КП и получивших любую дополнительную услугу в том же периоде."
& UNICHAR(10) & UNICHAR(10) &
"Повторные чекапы — количество КП, проданных пациентам, у которых ранее уже была куплена хотя бы одна КП."
```
